...
public partial class GreymarketCanvas : Control
{
    ...
    private void Build()
    {
        ...
        // TREES — framing woods at periphery/back corners only; village stays open
        for (int y = 0; y < GH; y++)
            for (int x = 0; x < GW; x++)
            {
                if (_terr[x, y] != 0) continue;
                float edge = Math.Min(Math.Min(x, GW - 1 - x), Math.Min(y, GH - 1 - y));
                bool backCorner = (x < 5 && y < 8) || (x < 8 && y < 4) || (x > 16 && y < 9) || (x > 18 && y < 13);
                bool periphery = edge < 2;
                if (!backCorner && !periphery) continue;
                if (x >= 7 && x <= 16 && y >= 8 && y <= 16) continue;
                uint h = Hash(x, y, 7);
                if ((h & 0xff) / 255f < 0.62f)
                {
                    float jx = (((h >> 8) & 0xff) / 255f - 0.5f) * 0.7f;
                    float jy = (((h >> 16) & 0xff) / 255f - 0.5f) * 0.7f;
    ...
    private void DrawGround()
    {
        for (int gy = 0; gy < GH; gy++)
            for (int gx = 0; gx < GW; gx++)
            {
                ...
                Vector2 Ex(Vector2 p) => ctr + (p - ctr) * 1.03f;
                var poly = new[] { Ex(c0), Ex(c1), Ex(c2), Ex(c3) };
                var key = GroundTex(t);
                var gtex = key != null ? View.Tex(key) : null;
                var light = KeyLight(gx + 0.5f, gy + 0.5f);
                uint h = Hash(gx, gy, 3);
                float j = ((h & 0xff) / 255f - 0.5f) * 0.06f;
                // focal falloff: brightest at the market (11,11), sinking to shadow at the edges so
                // the eye lands on the square. A warm lift near the centre, a cool sink far out.
                float dist = (Mathf.Abs(gx - 11) + Mathf.Abs(gy - 11)) / 18f;
                float focal = Mathf.Clamp(0.16f - dist * 0.5f, -0.22f, 0.16f);
                var foc = new Color(1 + focal, 1 + focal * 0.92f, 1 + focal * 0.74f);
                if (gtex != null)
                {
                    var mod = light * foc * new Color(1 + j, 1 + j, 1 + j);
                    if (t == 1) mod = mod * new Color("d8b27a");
                    if (t == 5) mod = mod * new Color("efd6a0");
                    DrawColoredPolygon(poly, mod, GroundUV, gtex);
                }
                else
                {
                    var col = GroundColor(t);
                    col = new Color(col.R * light.R * foc.R + j, col.G * light.G * foc.G + j, col.B * light.B * foc.B + j);
